...
blast_output = open("results_SM.txt", 'w')
...
for i in range(len(left)):
    ...
    for alignment in blast_result_record.alignments:
        for hsp in alignment.hsps:
            ...
            # Average bit score for SM matches
            ...
            # Continue if its not an sM
            if not (remove_dashes(hsp.sbjct) in right[i]):
                errors += 1
...
print(arrayofstats)
with open('stats_SM.csv', 'w') as mycsvfile:
